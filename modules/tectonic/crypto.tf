...
resource "tls_private_key" "ingress" {
  count     = "${var.existing_certs["ingress_cert_path"] == "/dev/null" ? 1 : 0 }"
  algorithm = "RSA"
  rsa_bits  = "2048"
}
...
resource "tls_locally_signed_cert" "ingress" {
  ...
  ca_key_algorithm   = "${var.existing_certs["ca_cert_path"] == "/dev/null" ? var.ca_key_alg : var.existing_certs["ca_key_alg"]}"
  ca_private_key_pem = "${var.existing_certs["ca_key_path"] == "/dev/null" ? var.ca_key : file(var.existing_certs["ca_key_path"])}"
  ca_cert_pem        = "${var.existing_certs["ca_cert_path"] == "/dev/null" ? var.ca_cert : file(var.existing_certs["ca_cert_path"])}"
