...
resource "tls_private_key" "apiserver" {
  count     = "${var.existing_certs["apiserver_cert_path"] == "/dev/null" ? 1 : 0}"
  algorithm = "RSA"
  rsa_bits  = "2048"
}
...
resource "tls_locally_signed_cert" "apiserver" {
  ...
  ca_key_algorithm   = "${var.existing_certs["ca_cert_path"] == "/dev/null" ? join(" ", tls_self_signed_cert.kube-ca.*.key_algorithm) : var.existing_certs["ca_key_alg"]}"
  ca_private_key_pem = "${var.existing_certs["ca_cert_path"] == "/dev/null" ? join(" ", tls_private_key.kube-ca.*.private_key_pem) : var.existing_certs["ca_key_path"]}"
  ca_cert_pem        = "${var.existing_certs["ca_cert_path"] == "/dev/null" ? join(" ", tls_self_signed_cert.kube-ca.*.cert_pem) : file(var.existing_certs["ca_cert_path"])}"
